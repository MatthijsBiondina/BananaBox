...
class View:
    ...
    def __init__(self, model):
        ...
        self.background = pygame.image.load("../res/maps/" + model.getMapName())
        self.layout = self.initNodeLayout()

    def initNodeLayout(self):
        layout = [ (self.nx + 4*32,self.ny + 15*32) ]
        for i in range(1,6):   layout.append( ( self.nx + 32*(i-1), self.ny))
        for i in range(6,11):  layout.append( ( self.nx + 32*(i-6), self.ny + 32) )
        for i in range(11,33): layout.append( ( self.nx + 96 + 32*((i-11)%2), self.ny + 96 + 32*int((i-11)/2)))
        for i in range(33,41): layout.append( ( self.nx + 192 + 32*((i-33)%4), self.ny + 32*int((i-33)/4)))
        for i in range(41,49): layout.append( ( self.nx + 352 + 32*((i-33)%4), self.ny + 32*int((i-41)/4)))
        for i in range(49,57): layout.append( ( self.nx + 32*((i-49)%2), self.ny + 96 + 32*int((i-49)/2)))
        for i in range(57,65): layout.append( ( self.nx + 32*((i-57)%2), self.ny + 320 + 32*int((i-57)/2)))

        for i in range(65,274): layout.append(( self.nx + 192 + 32*int((i-65)/14), self.ny + 96 + 32*int((i-65)/14)))
        for i in range(274,290): layout.append(( self.nx + 672 + 32*int((i-274)%3),self.ny + 96 + 32*int((i-274)/3)))
        return layout


    def start(self):
        sett = self.model.getSettings()
        self.gameDisplay = pygame.display.set_mode((int(sett['display_width']),int(sett['display_height'])))
    ...
    def drawNetwork(self):
        ...
        for c in connections:
            if c.getEnabled():
                self.drawAxon( c.getIn(), c.getOut(), network.getNode(c.getIn()).getActivity())
        
        nodes = network.getNodes()
        for n in range(len(nodes)):
            self.drawNode(nodes[n].getId(), self.layout[nodes[n].getId()], nodes[n].getActivity())
